...
def check_cookie_banner_with_claude(html: str, base_url: str = "") -> tuple:
    ...
    # Also fetch privacy page for more signals
    privacy_text = ""
    if base_url:
        for path in ["/datenschutz", "/datenschutzerklaerung", "/privacy", "/privacy-policy"]:
            try:
                r = requests.get(base_url + path, headers=HEADERS, timeout=6, allow_redirects=True)
                if r.status_code == 200:
                    privacy_soup = BeautifulSoup(r.text, "html.parser")
                    privacy_text = privacy_soup.get_text()[:3000]
                    break
            except:
                continue
    ...
    prompt = f"""Du analysierst eine deutsche Webseite auf Cookie-Consent.

Analysiere BEIDE Texte und beantworte: Hat diese Webseite ein Cookie-Consent-Banner oder eine Einwilligungslösung?

Hinweise die auf JA deuten:
- WordPress Plugin Pfade wie /plugins/complianz/, /plugins/cookie-law-info/, /plugins/borlabs-cookie/ im HTML
- Script-Tags mit cookie, consent, gdpr, cmplz, cli- im Dateinamen
- Datenschutzseite erwähnt Cookie-Einwilligung, Opt-in, Consent-Banner, Cookie-Zustimmung
- Begriffe wie "Sie können Ihre Einwilligung jederzeit widerrufen" oder "Cookie-Präferenzen"

HTML der Startseite:
{html_snippet}

Text der Datenschutzseite:
{privacy_text if privacy_text else "Nicht gefunden"}

Antworte NUR mit purem JSON, kein Markdown, keine Backticks:
{{"found": true/false, "reason": "kurze Begründung auf Deutsch (max 100 Zeichen)"}}"""
